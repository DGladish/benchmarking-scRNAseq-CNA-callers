...
print(paste("There are segmentation files from 2 algorithms being analyzed one chromosome at a time. Are the same number of windows used to analyze each chromosome?", identical(ascat_windows_per_chr[!(names(ascat_windows_per_chr) %in% c("chrX", "chrY"))], scevan_windows_per_chr)))
...
na_positions_all <- lapply(bin_wins, function(x) {which(is.na(x))})
na_positions_autosomes <- lapply(na_positions_all, function(x) {
  x <- x[!grepl("chrX", names(x))]
  return(x)
})

if (all(sapply(combn(seq_along(bin_wins), 2, simplify = FALSE),
               function(idx) identical(na_positions_autosomes[[idx[1]]], na_positions_autosomes[[idx[2]]])))) {print("NAs in ASCAT and SCEVAN vectors are all in the same position")
} else {
    print("NAs in the ASCAT vectors are not located at the same positions as NAs in the SCEVAN vectors")
}


###### Eliminate any element in the vector pairs that have an NA (i.e. if at least 1 window in a SCEVAN & ASCAT vector pair that corresponds to the same genomic position has value NA, eliminate the window from both vectors)
bin_wins_autosomes <- lapply(bin_wins, function(x) {
  x <- x[!grepl("chrX", names(x))]
  return(x)
})

na_positions_ascat_autosomes <- na_positions_autosomes[grep("ascat", names(na_positions_autosomes))][[1]]
na_positions_scevan <- na_positions_autosomes[grep("scevan", names(na_positions_autosomes))][[1]] # scevan by default only outputs autosomal data

na_positions_autosomes_union <- union(na_positions_ascat_autosomes, na_positions_scevan)

bin_wins_autosomes <- lapply(bin_wins_autosomes, function(x) {
  x <- x[-c(na_positions_autosomes_union)]
  return(x)
})
...
###### eliminate any windows associated with sex chromosomes & any windows where either SCEVAN or ASCAT contains an NA
ascat_bin_wins_autosomes_tg <- ascat_bin_wins_tg[-which(names(ascat_bin_wins_tg) %in% c("chrX", "chrY"))]

na_positions_ascat_tg <- which(is.na(ascat_bin_wins_autosomes_tg))
na_positions_scevan_tg <- which(is.na(scevan_bin_wins_tg))
na_positions_union_tg <- union(na_positions_ascat_tg, na_positions_scevan_tg)

ascat_bin_wins_autosomes_tg <- ascat_bin_wins_autosomes_tg[-na_positions_union_tg]
scevan_bin_wins_tg <- scevan_bin_wins_tg[-na_positions_union_tg]
